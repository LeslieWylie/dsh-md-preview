...
//   * `/dsh-md-html-render` — a private RPC channel the web client half calls,
...
// path to disk.
//
//   /dsh-md-html-render  ls           { path? }                      -> { ok, path, items }
...
export const name = 'dsh-md-html-render'
...
const CHANNEL = '/dsh-md-html-render'
...
function registerRenderTool(ctx, fs) {
  ...
  import('@deepseek-ai/dsh-tools')
    ...
    .catch((error) => {
      ctx.logger?.warn('dsh-md-html-render: md_html_render unavailable (' + errorText(error) + ')')
    })
...
export function apply(ctx) {
  const fs = ctx.fs
  if (fs === undefined) {
    ctx.logger?.warn('dsh-md-html-render: no fs service; the plugin will stay inert')
    return
  }
